...
# important note:
# because of pagination qs of ideas:
# reponse.data is collections.OrderedDict' so to get amount ideas use key ='count'
# otherwise set pagination class in settings: should be None and work with length qs
# ex: OrderDict ([ ('count',2),('next',None),('previous',None),('results',[OrderDict([('id':...)])])])
    ...
    def test_get_idea_for_category_without_children(self):
        ...
        # only if working with qs, but above I get OrderDict as resp.data
        #self.assertEqual(local_serialized_data, response.data)
